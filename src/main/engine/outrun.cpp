...
void Outrun::vint()
{
    ...
    if (config.fps < 120 || (cannonball::frame & 1))
    {
        ...
        // [ND] outputs->coin_chute_out(&outputs->chute1, coin == 1);
        // [ND] outputs->coin_chute_out(&outputs->chute2, coin == 2);
...
void Outrun::jump_table(Packet* packet)
{
    ...
    if (config.cannonboard.enabled && config.cannonboard.debug)
    {
        ...
        // [ND] ohud.blit_text_new(x, y, "MC OUT");    ohud.blit_text_new(x + 10, y, Utils::to_hex_string(outputs->hw_motor_control).c_str(), OHud::PINK); x += 13;
        ...
        // [ND] ohud.blit_text_new(x, y, "DIG OUT"); ohud.blit_text_new(x + 10, y, Utils::to_hex_string(outputs->dig_out).c_str(), OHud::PINK); x += 13;
    }
}
